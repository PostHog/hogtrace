...
impl<'py> PythonDispatcher<'py> {
    ...
    fn get_arg_n(&self, n: usize) -> Result<Value, String> {
        ...
        if let Ok(args_obj) = locals.get_item("args")
            && let Ok(args_tuple) = args_obj.cast_into::<PyTuple>()
            && let Ok(arg) = args_tuple.get_item(n)
        {
...
impl<'py> Dispatcher for PythonDispatcher<'py> {
    fn load_variable(&mut self, name: &str) -> Result<Value, String> {
        ...
        match name {
            ...
            _ => {
                ...
                if let Some(stripped) = name.strip_prefix("arg")
    ...
    fn store_variable(&mut self, name: &str, _value: Value) -> Result<(), String> {
    ...
    fn get_attribute(&mut self, obj: &Value, attr: &str) -> Result<Value, String> {
        ...
        if let Value::Object(boxed) = obj
            && boxed.downcast_ref::<RequestStoreProxy>().is_some()
        {
    ...
    fn set_attribute(&mut self, obj: &Value, attr: &str, value: Value) -> Result<(), String> {
        ...
        Err(format!(
            "Cannot set attribute {} on non-request object",
            attr
        ))
    }
    ...
    fn call_function(&mut self, name: &str, args: Vec<Value>) -> Result<Value, String> {
        ...
        match name {
            ...
            "len" => {
                ...
                let len = obj.len().map_err(|e| format!("len() failed: {}", e))?;
            ...
            _ => {
                ...
                let py_args: Result<Vec<Bound<'py, PyAny>>, String> =
                    args.iter().map(|v| self.value_to_py(v)).collect();
                let py_args = py_args?;
